<?= $this->extend('dashboard/dashboard') ?>
<?= $this->section('page-content') ?>
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Marketplace</h1>
<!-- DataTales Example -->
<?php if ($toko) : ?>
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-danger">Detail Market</h6>
            <div class="dropdown no-arrow">
                <a class="dropdown-toggle" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="fas fa-ellipsis-v fa-sm fa-fw text-gray-400"></i>
                </a>
                <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink" style="">
                    <a class="dropdown-item" href="#"><i class="bi bi-pencil-square"></i> Edit Detail</a>
                </div>
            </div>
        </div>
        <div class="card-body">

            <div class="row">
                <div class="col-xl-3 col-md-6 mb-4">
                    <div class="card border-left-danger shadow h-100 py-2">
                        <div class="card-body">
                            <div class="row no-gutters align-items-center">
                                <div class="col mr-2">
                                    <div class="text-xs font-weight-bold text-danger text-uppercase mb-1">
                                        ID Market</div>
                                    <div class="h5 mb-0 font-weight-bold text-gray-800"><?= $toko['id_toko']; ?></div>
                                </div>
                                <div class="col-auto">
                                    <i class="bi bi-shop fa-2x text-gray-300"></i>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <div class="card shadow mb-4">
                        <!-- Card Header - Accordion -->
                        <a href="#deskripsi" class="d-block card-header py-3" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="deskripsi">
                            <h6 class="m-0 font-weight-bold text-danger">Deskripsi</h6>
                        </a>
                        <!-- Card Content - Collapse -->
                        <div class="collapse" id="deskripsi" style="">
                            <div class="card-body">
                                <?= $toko['deskripsi']; ?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <div class="card shadow mb-4">
                        <!-- Card Header - Accordion -->
                        <a href="#alamat" class="d-block card-header py-3" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="alamat">
                            <h6 class="m-0 font-weight-bold text-danger">Alamat</h6>
                        </a>
                        <!-- Card Content - Collapse -->
                        <div class="collapse show" id="alamat">
                            <div class="card-body">
                                <p>Telepon : <?= $toko['telp']; ?></p>
                                <?= ($toko['telp2']) ? '<p>Telepon : ' . $toko['telp2'] . '</p>' : ''; ?>
                                <p>
                                    Provinsi : <?= $toko['province']; ?>
                                </p>
                                <p>
                                    Kabupaten : <?= $toko['city']; ?>
                                </p>
                                <p>
                                    Kode Pos : <?= $toko['zip_code']; ?>
                                </p>
                                <p><?= $toko['alamat_1']; ?></p>
                                <?php if ($toko['alamat_2']) : ?>
                                    <p>Detail alamat :</p>
                                    <p><?= $toko['alamat_2']; ?></p>
                                <?php endif ?>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <div class="card shadow mb-4">
                        <!-- Card Header - Accordion -->
                        <a href="#user" class="d-block card-header py-3" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="user">
                            <h6 class="m-0 font-weight-bold text-danger">Informasi User</h6>
                        </a>
                        <!-- Card Content - Collapse -->
                        <div class="collapse" id="user" style="">
                            <div class="card-body">
                                <!-- // -->
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
<?php else : ?>
    <?php
    header('Location:' . base_url('dashboard/marketplace/create'));
    exit; ?>
<?php endif ?>
<?= $this->endSection(); ?>