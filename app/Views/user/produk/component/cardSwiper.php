<div class="container" id="best">
    <div class="swiper testing mt-5">
        <h2> Produk Terbaik</h2>
        <div class=" swiper-wrapper col-lg-2">
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide-card" style="margin: 10px;">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
            <div class="swiper-slide">
                <div class="card border-0 shadow" style="width: 10rem;">
                    <img src="<?= base_url() ?>assets/img/produk/p5.png" style="height: auto; width: 150px;" class="card-img-top" alt="...">
                    <div class="card-body text-center">
                        <h6 class="card-text">Rp. 25.000</h6>
                        <p class="card-title">Norigo Rasa Keju</p>
                        <a href="#" class="btn btn-white text-danger mt-2"> <i class="fas fa-shopping-cart"></i></a>
                        <a href="#" class="btn btn-white text-danger mt-2"> Beli</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Swiper JS -->
<script src="https://cdn.jsdelivr.net/npm/swiper@10/swiper-bundle.min.js"></script>
<script src="<?= base_url() ?>assets/js/swiper.js"></script>